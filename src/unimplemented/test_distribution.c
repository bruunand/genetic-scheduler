...
{
    int i, courseNum, weekNum, dayNum, totCoursePerWeek, totCoursePerDay, totLectPerWeek, totDays, severity = 0;
    ...
    for (courseNum = 0; courseNum < numCourses, courseNum++)
        for (weekNum = 0, dayNum = 0; dayNum < totDays; dayNum++)
        {
            ...
            for (i = 0, totCoursePerWeek = 0, totCoursePerDay = 0, totLectPerWeek = 0; i < sd->numLectures); i++)
            {
                if (sd->lectures[i].day / DAYS_PER_WEEK == weekNum && sd->lectures[i].assignedCourse == 0 && sd->lectures[i].assignedCourse)
                    totLectPerWeek++;
